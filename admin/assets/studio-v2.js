...
const state={rows:[],polling:new Set(),serviceReady:false,cloudTimer:null,cloudBusy:false,cloudFailures:0,lastSyncAt:0};
...
async function runCloudPoll(){if(Store.localOnly||state.cloudBusy)return;state.cloudBusy=true;try{const rows=await syncJobs();state.lastSyncAt=Date.now();state.cloudFailures=0;global.dispatchEvent(new CustomEvent('eastudy:studio-sync',{detail:{ok:true,at:state.lastSyncAt}}));state.cloudTimer=setTimeout(runCloudPoll,rows.some(job=>['QUEUED','PROCESSING','RUNNING','WAITING'].includes(job.status))?5000:20000)}catch(error){state.cloudFailures++;global.dispatchEvent(new CustomEvent('eastudy:studio-sync',{detail:{ok:false,error}}));const delay=Math.min(30000,5000*2**Math.min(3,state.cloudFailures-1));state.cloudTimer=setTimeout(runCloudPoll,delay+Math.random()*500)}finally{state.cloudBusy=false}}
