-- Keep source uncertainty explicit without discarding reliable teaching rows.
-- No transcript/timing correction is inferred from a language-model concern.
begin;
...
begin
  ...
  if jsonb_typeof(translation) is distinct from 'object'
    or coalesce(translation->>'status','') not in ('completed','source_unresolved')
  ...
  if (translation->>'status'='completed') is distinct from (jsonb_array_length(translation->'sourceConcerns')=0)
    then return false; end if;
  if translation->>'status'='source_unresolved' and coalesce(translation->>'translationOrigin','')
    not in ('retained_source','reviewed_candidate') then return false; end if;
