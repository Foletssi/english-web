...
本轮只新增 `audit/local_intake_acceptance.py` 并更新本记录，没有修改线上业务代码，不需要重新发布相同应用版本。生产只读复核：Worker 2.5.0 心跳在线、localInputV1=true；处理任务仅一条 CANCELLED，无在途任务。

可复现命令（使用有足够空间的本地测试目录）：

```powershell
py -3.12 audit/local_intake_acceptance.py --scratch-parent <测试目录>
```

实测使用隔离 SQLite、本机随机端口 HTTP 和真实 FFmpeg；测试目录用完自动清理，不读取生产凭据、不发云端请求、不处理旧视频、不删除 R2 对象。400 MiB 输入为 5 秒 720P/60fps 合成视频加有效 MP4 free 填充，不代表 400 MiB 真实长视频。云端票据和 READY 确认使用模拟边界。

| 验收点 | 实测结果 |
| --- | --- |
| 同块重复上传 | 只保留一个成功块记录 |
| HTTP 请求体中断 | 返回 BODY_INCOMPLETE，未记成功 |
| 有序停止并重启接收器 | 已确认块保留；不代表强杀进程测试 |
| 已收块损坏 | 状态校验剔除损坏块，重传成功 |
| 400 MiB 分块组装 | 完整 SHA256、封面字节和真实媒体探测通过 |
| 模拟云端确认失败 | READY 状态保留，再次发送确认成功 |
| 媒体制作与复用 | 独立 ffprobe 确认实际 960×540/30fps；PCM 单声道16kHz/16bit；视频编码首次1次、复用0次；复用后产物摘要和修改时间不变 |

修订后的复测：传输含中断／重启恢复 3.212 秒，5秒测试媒体制作含探测 1.419 秒，脚本总计 8.649 秒。上述数字仅用于本机隔离测试，不是互联网速度、长视频耗时或完整 AI 制作吞吐。输出中的一次 ConnectionError 是主动注入的确认丢失，不是未处理失败。

### Standards

本轮独立审查未发现文档规范违例或需处理的代码异味；指出一项 P2 测试可靠性问题：30fps原断言读取编码配置，不能证明实际产物帧率。已改用独立 ffprobe 检测实际产物，并重新运行通过。

### Spec

独立审查指出同一项 P2 帧率证据问题，已修正；补充记录视频编码调用首次1次、缓存复用0次。未发现本轮超范围改动。仍未完成真实 Chrome 交互、真实长原片、强杀进程恢复和新视频 ASR→DeepSeek→发音→生产上传→学生消费的完整验收；GPU发音吞吐也没有新增实测证据。

双轴初审各1项P2（同一问题），修复复测通过；完整规格验收仍为部分完成。浏览器最新重试返回空浏览器列表及 `Browsers: Error: nodeRepl.fetch request failed`，当前错误不是先前的 apikey 认证报错。需恢复Chrome扩展与当前Codex会话的连接，才能继续真实页面验收，不能以此脚本替代。
