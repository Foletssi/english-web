(function(global){
  ...
  function prepareCreator(input){const id=input.id||'creator-'+Date.now(),previous=load().creators.find(x=>String(x.id)===String(id))||{};return {...previous,...deep(input),id,name:validCreatorName(input.name??previous.name),status:input.status||previous.status||'ACTIVE',deletedAt:input.status==='ACTIVE'?null:(input.deletedAt??previous.deletedAt)}}
  function saveCreator(input){const next=prepareCreator(input),s=load(),id=next.id,idx=s.creators.findIndex(x=>String(x.id)===String(id));if(idx>=0)s.creators[idx]=next;else s.creators.unshift(next);save(s,{type:'creator.save',entityId:id,name:next.name});return deep(next)}
  ...
  function prepareCollection(input){const id=input.id||Date.now(),previous=load().collections.find(x=>String(x.id)===String(id))||{};return {...previous,...deep(input),id}}
  function saveCollection(input){const next=prepareCollection(input),s=load(),id=next.id,idx=s.collections.findIndex(x=>String(x.id)===String(id));if(idx>=0)s.collections[idx]=next;else s.collections.unshift(next);save(s,{type:'collection.save',entityId:id,title:next.title});return deep(next)}
