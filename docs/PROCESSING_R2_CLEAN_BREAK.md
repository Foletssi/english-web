...
旧 Job 迁移使用临时环境变量运行 `node scripts/migrate-processing-to-r2.mjs`。脚本按 Job ID 幂等导入，不会重复创建任务、重复调用 DeepSeek 或删除 R2 对象。导入完成后重启 Worker，使其使用 `https://english-web-lce.pages.dev/api/processing/control`。
