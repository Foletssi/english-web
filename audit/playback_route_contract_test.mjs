...
const rangeVoice='voice/'+ 'b'.repeat(64)+'.mp3';
const voiceRangeRequest=(range,cookie=coverCookie)=>{
 const context=cacheRequest(rangeVoice,cookie);
 context.request.headers.set('Range',range);return context;
};
const readsBeforeRanges=reads;
for(const [range,expectedCache,start,end] of [['bytes=0-1','MISS',0,1],['bytes=2-','HIT',2,body.length-1],['bytes=0-999','HIT',0,body.length-1]]){
 const response=await readMedia(voiceRangeRequest(range));
 assert.equal(response.status,206);
 assert.equal(response.headers.get('X-Eastudy-Media-Cache'),expectedCache);
 assert.equal(response.headers.get('Content-Range'),`bytes ${start}-${end}/${body.length}`);
 assert.equal(response.headers.get('Content-Length'),String(end-start+1));
 assert.equal(response.headers.get('Cache-Control'),'private, no-store');
 assert.deepEqual(new Uint8Array(await response.arrayBuffer()),body.slice(start,end+1));
}
assert.equal(reads,readsBeforeRanges+1,'mobile range probes share one full edge object');
const wholeAfterRange=await readMedia(cacheRequest(rangeVoice,coverCookie));
assert.equal(wholeAfterRange.status,200);assert.equal(await wholeAfterRange.text(),'#EXTM3U\n','range responses never poison the full cache entry');
assert.equal((await readMedia(voiceRangeRequest('bytes=999-'))).status,416);
canPlay=false;const beforeRevokedRange=cacheReads;
assert.equal((await readMedia(voiceRangeRequest('bytes=0-1'))).status,403);
assert.equal(cacheReads,beforeRevokedRange,'cached byte ranges still require active membership');canPlay=true;
assert.equal((await readMedia(voiceRangeRequest('bytes=0-1',''))).status,401);
const normalPut=globalThis.caches.default.put;
globalThis.caches.default.put=async()=>{throw Error('cache full')};
assert.equal((await readMedia(cacheRequest('voice/'+ 'c'.repeat(64)+'.mp3',coverCookie))).status,200,'cache write failure must not fail authorized audio delivery');
globalThis.caches.default.put=normalPut;
const normalMatch=globalThis.caches.default.match;
globalThis.caches.default.match=async()=>{throw Error('cache unavailable')};
const fallbackRange=await readMedia(voiceRangeRequest('bytes=0-1'));
assert.equal(fallbackRange.status,206,'cache read failure falls back to authorized origin');
assert.equal(fallbackRange.headers.get('X-Eastudy-Media-Cache'),'MISS');
assert.deepEqual(new Uint8Array(await fallbackRange.arrayBuffer()),body.slice(0,2));
globalThis.caches.default.match=normalMatch;
