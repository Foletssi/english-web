...
assert.ok(app.includes('void openDict(target,event)'), 'word-card clicks intercept sentence navigation before it runs');
assert.ok(app.includes("const DICTIONARY_TRIGGER_SELECTOR='.word-token,.teaching-expression,.insight-word';"), 'all word-card entry points share one trigger selector');
assert.ok(app.includes("root.addEventListener('pointerup'"), 'touch word taps open cards before sentence navigation');
assert.ok(app.includes('button.className=\'insight-word teaching-expression\''), 'current-sentence insight words are actual card triggers');
