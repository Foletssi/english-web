...
import { createOrImportJob, claimJob, updateJob, recordReceipts, controlAssetKey } from '../functions/_lib/r2-processing.js';
class Obj { constructor(text, etag){this.value=text;this.etag=etag;} async text(){return this.value;} }
class Bucket { constructor(){this.store=new Map();this.seq=0;} async get(key){const item=this.store.get(key);return item?new Obj(item.body,item.etag):null;} async put(key,body,options={}){const old=this.store.get(key);if(options.onlyIf?.etagMatches && (!old||!options.onlyIf.etagMatches.includes(old.etag)))return null;const etag='etag-'+(++this.seq);this.store.set(key,{body:String(body),etag});return {etag};} }
...
assert.equal(done.status,'REVIEW'); assert.equal(done.output_run_id,done.run_id); assert.equal(done.receipt_count,1); assert.match(controlAssetKey(jobId,done.run_id,'voice/a.mp3'),new RegExp(jobId));
