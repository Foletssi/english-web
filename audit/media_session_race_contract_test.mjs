...
const context=vm.createContext({window,fetch,console,setTimeout,clearTimeout,Headers,Response,AbortController,CustomEvent:class{},Date});
...
const lateOldPost=finishPost;
let nextAccountStarted=false;
const newSync=window.EastudyCloudContent.syncMediaSession('student').then(()=>{nextAccountStarted=true});
await new Promise(resolve=>setTimeout(resolve,0));
assert.equal(finishPost,lateOldPost,'new session must wait for old cookie cleanup');
finishPost();
await stuckSync;
await new Promise(resolve=>setTimeout(resolve,0));
...
finishPost();
await newSync;
assert.equal(nextAccountStarted,true);
