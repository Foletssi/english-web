...
(()=>{let snapshot=${JSON.stringify(snapshot)},revision=8,trash=[];window.__rpcCalls=[];
 const ok=data=>Promise.resolve({data,error:null}),api={auth:{getSession:()=>ok({session:{access_token:'fixture-token'}})},rpc(name,args={}){window.__rpcCalls.push({name,args});
  ...
  if(name==='admin_save_content_snapshot'){snapshot=args.p_snapshot;revision++;return ok([{revision,updated_at:new Date().toISOString()}])}
...
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true});try{
 ...
 assert.equal(calls.filter(x=>x.name==='admin_save_content_snapshot').length,1,'pending draft save must flush before delete');
