...
export async function onRequestPost({ request, env }) {
  ...
  let access;
  try {
    access = await serviceRpc(env, 'service_get_user_learning_access_v2', { p_user_id: auth.user.id });
  } catch (error) {
  ...
  const now = Math.floor(Date.now() / 1000),exp=playback.kind==='ADMIN'?now+300:Number.isFinite(entitlementExpiry)?Math.min(now+300,Math.floor(entitlementExpiry)):now,prefix=String(playback.prefix||'');
  if(!prefix||objectKey!==prefix+'720p/index.m3u8')return json({error:'PLAYBACK_FORBIDDEN'},403);
